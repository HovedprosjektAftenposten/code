...
var fokusHendelse = 0,		//Representerer indexen av hendelsen leseren leser
    $headerHeight = 0,		//Headeren har dynamisk høyde, og vi trenger høyden flere ganger for å scrolle riktig ved event.
    $ingressHeight = 0,	    //Brukes for å finne ut høyden på header uten ingress-tekst.
    $boolIngress = true,	//True = Ingressteksten er synlig.
    kart = false, 			//Er det et kart i tidslinjen? 
    map,
    maparray = [],
    markerarray = [];
...
function hentDataPrototype(){
	...
	$.getJSON(url, function(jsonData){
		...
		$.each(jsonPrototype, function(i,item){//First indent
            ...
            var timeline_title = item.tl_title;
            var timeline_ingress = item.tl_ingress;
            htmlHeaderTxt += "<div class='nu-timeline-header-wrapper'>";
            ...
            $.each(item.tl_content, function(i, item){//Second indent
                ...
                var custom_time = item.custom_time;
                var important = item.content_important;            
    			...
    			var hendelseDato = new Date(nytt_format_dato + " " + time); //Makes a Dateobject, with clocktime. Should be suitable for both webkit and ie8.
...
function leggTilIndikatorer(datoArray, importantDates, custom_time, categories){
    ...
    for (var i = 0; i < datoArray.length; i++){
        ...
        //A class that represent important dates is added
        var importantClass = "";
        if (importantDates[i] == "true")
        {        
        ...
        if(prosent < 5){           
             $(".nu-timeline-indikator-open[data-date-percent='" + prosent + "']").css({"text-align":"left","margin-left":"0" });
        }else if(prosent > 95){
             $(".nu-timeline-indikator-open[data-date-percent='" + prosent + "']").css({"text-align":"right","margin-left":"0","right":"0" });
        }
...
function hentData(){
    ...
    $.getJSON(prototypeUrl, function(jsonData){
        ...
        var importantDates = []; //Makes ready an array for the important dates
        var categoryDates = []; //Makes ready an array that holds the category for each date
    ...
    $(document).bind('keydown',function(evt) {
        ...
        switch(evt.keyCode) {
            ...
            case 40:
                        ...
                        break;
            case 32:    
                        evt.preventDefault(); //Default space-keydown will jump down the document. Prevent.
                        nextArticle();
                        break;
            case 37:
                        previousArticle();
